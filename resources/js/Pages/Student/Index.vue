	...
	<app-layout title="Dashboard">
		<div>
			<div class="flex justify-between">
				...
				<Link :href="route('students.create')" class="bg-cyan-300 px-2 py-2 text-white rounded-md"> Create </Link>
			</div>
								...
								<tr v-for="(student, index) in students" :key="index" class="bg-white">
									...
									<td class="px-3 py-2 lg:px-6 lg:py-3 text-right whitespace-nowrap text-sm text-gray-900">
										<div class="flex justify-end space-x-2 text-right">
											...
											<button @click="openModel(student.id)">
...
export default defineComponent({
	...
	setup(props) {
		...
		function deleteRole() {
			Inertia.delete(route("students.destroy", student.value), {
				preserveState: true,
				preserveScroll: true,
				onSuccess: (page) => {
					open.value = false
