			...
			<div class="flex justify-between">
				<Breadcrumb
					:links="[
						...
						{title: 'Courses', url: '/marks'},
					]"
				/>

				<Link :href="route('courses.create')" class="bg-cyan-300 px-2 py-2 text-white rounded-md"> Create </Link>
			</div>
					...
					<div class="align-middle min-w-full overflow-x-auto shadow overflow-hidden rounded-md sm:rounded-lg">
						<table class="min-w-full divide-y divide-gray-200">
							...
							<tbody class="bg-white divide-y divide-gray-200">
								<tr v-for="(course, index) in courses" :key="index" class="bg-white">
									...
									<td class="px-3 py-2 lg:px-6 lg:py-3 text-right whitespace-nowrap text-sm text-gray-900">
										<div class="flex justify-end space-x-2 text-right">
											...
											<button @click="openModel(course.id)">
							...
							<div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
								<div class="sm:flex sm:items-start">
									...
									<div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
										<DialogTitle as="h3" class="text-lg leading-6 font-medium text-gray-900">Delete Student? </DialogTitle>
										<div class="mt-2">
											<p class="text-sm text-gray-500">Are you sure you want to delete the Student? All of your data will be permanently removed. This action cannot be undone.</p>
										</div>
									</div>
								</div>
							</div>
...
export default defineComponent({
	...
	setup(props) {
		...
		function deleteRole() {
			Inertia.delete(route("courses.destroy", course.value), {
				preserveState: true,
				preserveScroll: true,
				onSuccess: (page) => {
					open.value = false
